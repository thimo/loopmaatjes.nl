...
class Envato_Backup {
  ...
  public function pcl_zip() {
    ...
    /* Zip up everything */
    if ( ! $archive->add( $this->root(), PCLZIP_OPT_REMOVE_PATH, $this->root(), PCLZIP_CB_PRE_ADD, 'envato_backup_pclzip_callback' ) )
  ...
  public function check_archive() {
    ...
    if ( ! file_exists( $this->archive_filepath() ) )
      $this->error( $this->archive_method(), __( 'The backup file was not created', 'gp_lang' ) );
  ...
  public function files() {
    ...
    if ( ! empty( $this->unreadable_files ) )
      $this->warning( $this->archive_method(), __( 'The following files are unreadable and could not be backed up: ', 'gp_lang' ) . implode( ', ', $this->unreadable_files ) );
 ...
 * @return    bool
 */
function envato_backup_pclzip_callback( $event, &$file ) {
