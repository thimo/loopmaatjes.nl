...
class Envato_WP_Toolkit {
        ...
        if ( empty( $errors ) && count( $themes ) > 0 ) {
          ...
          foreach( $themes as $theme ) {
            ...
            if ( $stylesheet && $template && $current_stylesheet !== $stylesheet ) {
              ...
              $links[] = '<a href="' . $delete_url . '" class="submitdelete deletion" title="' . esc_attr( sprintf( __( 'Delete &#8220;%s&#8221;', 'gp_lang' ), $title ) ) . '" ' . $delete_onclick . '>' . __( 'Delete', 'gp_lang' ) . '</a>';
            ...
            /* Tags list */
            if ( $tags ) {
              $content.= '<p>' . __( 'Tags: ', 'gp_lang' ). join( ', ', $tags ) . '</p>';
            }
          ...
          if ( ! empty( $premium_themes ) ) {
            ...
            if ( ! empty( $current_theme ) ) {
              _e( '<h3>Current Theme</h3>', 'gp_lang' );
            ...
            /* list premium themes */
            _e( '<h3>Available Themes</h3>', 'gp_lang' );
            echo '<ul class="ewpu-item-list">';
            foreach ( $premium_themes as $k => $v )
  ...
  public function _http_request_args( $r ){
    if ( (int) ini_get( 'max_execution_time' ) <  EWPT_PLUGIN_MAX_EXECUTION_TIME ) {
      ini_set( 'max_execution_time', EWPT_PLUGIN_MAX_EXECUTION_TIME ); // MODIFIED
    }
